...
class FeedbackService:
    ...
    def _load_quotas(self):
        ...
        if self.quota_file.exists():
            self._quotas = json.loads(self.quota_file.read_text(encoding="utf-8"))
        else:
            self._quotas = {}

    def _save_quotas(self):
        """保存用户额度数据"""
        self.quota_file.write_text(json.dumps(self._quotas, ensure_ascii=False, indent=2), encoding="utf-8")

    def get_user_quota(self, username: str) -> dict:
        """获取用户检测额度和反馈统计"""
        q = self._quotas.get(username, {"feedback_count": 0, "bonus_quota": 0, "total_earned": 0})
        return {
            "username": username,
            "feedback_count": q.get("feedback_count", 0),
            "bonus_quota": q.get("bonus_quota", 0),
            "total_earned": q.get("total_earned", 0),
            "next_reward_at": self.REWARD_INTERVAL - (q.get("feedback_count", 0) % self.REWARD_INTERVAL),
        }

    def record_feedback_reward(self, username: str) -> dict:
        """记录反馈并计算奖励（每次提交反馈后调用）"""
        if username not in self._quotas:
            self._quotas[username] = {"feedback_count": 0, "bonus_quota": 0, "total_earned": 0}

        q = self._quotas[username]
        q["feedback_count"] = q.get("feedback_count", 0) + 1

        # 检查是否达到奖励阈值
        rewarded = False
        if q["feedback_count"] % self.REWARD_INTERVAL == 0:
            q["bonus_quota"] = q.get("bonus_quota", 0) + self.REWARD_BONUS
            q["total_earned"] = q.get("total_earned", 0) + self.REWARD_BONUS
            rewarded = True

        self._quotas[username] = q
        self._save_quotas()
        ...
        return {
            ...
            "rewarded": rewarded,
            "reward_amount": self.REWARD_BONUS if rewarded else 0,
            "next_reward_at": self.REWARD_INTERVAL - (q["feedback_count"] % self.REWARD_INTERVAL),
        }

    def get_leaderboard(self, limit: int = 10) -> List[dict]:
        """获取反馈排行榜"""
        entries = []
        for username, q in self._quotas.items():
            entries.append({
                "username": username,
                "feedback_count": q.get("feedback_count", 0),
                "bonus_quota": q.get("bonus_quota", 0),
        ...
        entries.sort(key=lambda x: x["feedback_count"], reverse=True)
