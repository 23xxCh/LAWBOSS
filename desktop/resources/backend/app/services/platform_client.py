...
class LazadaClient(BasePlatformClient):
    ...
    def __init__(self):
        ...
        self.seller_id = os.getenv("LAZADA_SELLER_ID", "")
        self.enabled = bool(self.app_key and self.app_secret and self.access_token)
    ...
    async def fetch_listings(
        ...
        if not self.enabled:
            logger.warning("Lazada API 未配置")
            return []

        try:
            import httpx
            base_urls = {
                "SEA_SG": "https://api.lazada.com.sg",
                "SEA_TH": "https://api.lazada.co.th",
                "SEA_MY": "https://api.lazada.com.my",
                "SEA_PH": "https://api.lazada.com.ph",
                "SEA_ID": "https://api.lazada.co.id",
                "SEA_VN": "https://api.lazada.vn",
            }
            base_url = base_urls.get(market, base_urls["SEA_SG"])

            async with httpx.AsyncClient(timeout=30.0) as client:
                resp = await client.get(
                    f"{base_url}/rest/products/get",
                    params={
                        "app_key": self.app_key,
                        "access_token": self.access_token,
                        "offset": 0,
                        "limit": limit,
                        "filter": "all" if status == "active" else status,
                    },
                )
                if resp.status_code == 200:
                    return self._parse_items(resp.json(), market)
    ...
    def _parse_items(self, data: dict, market: str) -> List[PlatformListing]:
        ...
        for item in data.get("data", []):
            listings.append(PlatformListing(
                listing_id=str(item.get("item_id", "")),
                platform="lazada",
                title=item.get("attributes", {}).get("name", ""),
                description=item.get("attributes", {}).get("description", ""),
                category=self.map_category(item.get("category_name", "")),
                market=market,
                item_id=str(item.get("item_id")),
                sku=item.get("skus", [{}])[0].get("SellerSku") if item.get("skus") else None,
                price=float(item.get("skus", [{}])[0].get("price", 0)) if item.get("skus") else None,
...
class TikTokShopClient(BasePlatformClient):
    ...
    def __init__(self):
        ...
        self.enabled = bool(self.app_key and self.app_secret and self.access_token and self.shop_id)
    ...
    async def fetch_listings(
        ...
        if not self.enabled:
            logger.warning("TikTok Shop API 未配置")
            return []
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                resp = await client.get(
                    f"{base_url}/api/v1/product/list",
                    headers=headers,
                    params={
                        "shop_id": self.shop_id,
                        "page_size": limit,
            ...
            logger.error(f"TikTok Shop 拉取失败: {e}")
            return []
    ...
    def _parse_items(self, data: dict, market: str) -> List[PlatformListing]:
        ...
        for item in data.get("data", {}).get("products", []):
            listings.append(PlatformListing(
                listing_id=str(item.get("product_id", "")),
                ...
                price=float(item.get("price", {}).get("original_price", 0)) if item.get("price") else None,
