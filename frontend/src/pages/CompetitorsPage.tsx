...
function Space({ children, align, size }: { children: React.ReactNode; align?: 'start' | 'end' | 'center'; size?: 'small' | 'middle' | 'large' }) {
  const gap = size === 'large' ? 24 : size === 'middle' ? 16 : 8;
  return <div style={{ display: 'flex', alignItems: align || 'center', gap }}>{children}</div>;
}
